...
# global variables for twitter interactions
consumer_key = 'wIDHvofdfV2QO94s1bjebQ'
consumer_secret = 'nO0q0Ko8EBQ6Lb8FNLwEsT3r2QLkjWsO02dr9uegU'
access_token_key = '2408639030-qbEH2eTP5Qc0fJU9NfhDRREBB9F44cxhGSFbV3T'
access_token_secret = 'xPLGzRWpsPeXCFzw6PI98K91XegwKAFFq0JPjgTJRj6ge'
user = 'Teo'

def tweet_removedtask(string):
    '''
    :param string: text of just removed task

    This method tweets a removed task. It adds the following information:
    - 2 hashtags "#AmI2016" and "#removedTaskFor<YourName>" (substitute <YourName> with your name)
    - A mention to the AmI course inserting "@AmI_course"

    '''

    # get access to python-twitter apis
    api = twitter.Api(consumer_key=consumer_key,
                      consumer_secret=consumer_secret,
                      access_token_key=access_token_key,
                      access_token_secret=access_token_secret)
    hash_tags = '@Ami_course #AmI2016 #removedTaskFor'+user+' '
    max_string_len = 140 - len(hash_tags)
    short_string = string[:max_string_len]
    tweet = hash_tags + short_string
    api.PostUpdate(tweet)
    print "this tweet was just posted: '"+tweet+"'\n"


def import_from_twitter(tasks_list):
    '''
    :param tasks_list: list of existing tasks
    :return updated list of tasks

    This method import the 20 most recent mentions (tweets containing your username
     in the form @username) that contain:
    - the hashtag "#AmI2016"
    - the hashtag "#addedTaskFor<YourName>" (substitute <YourName> with your name)
    ...
    for mention in mentions:
        ...
        if ("#AmI2016" in mention.text and "#addedTaskFor"+user in mention.text):
            # encode the obtained string
            task = mention.text.encode('utf-8')
            # remove tags and hashtags
            task = task.replace("@AmI_course ", "")
            task = task.replace("#AmI2016 ", "")
            task = task.replace("#addedTaskFor"+user+" ", "")
            # add the task to the existing list
            tasks_list.append(task)

            print "task '"+task+"' just added to the list\n"
    return tasks_list
    ...
    if (len(remove_list) > 0):
        for task_to_remove in remove_list:
            if (task_to_remove in tasks_list):
                ...
                tweet_removedtask(task_to_remove)
    else:
...
if __name__ == '__main__':
    ...
    while not ended:
        ...
        print "6: import last 20 tasks from twitter"
        ...
        elif (choice == 5):  # read the first 2 tasks
            read_2_tasks(tasks_list)
        elif (choice == 6):  # import last 20 tasks from twitter
            import_from_twitter(tasks_list)
        elif (choice == 7):  # exit
            ended = True
